...
class SpeechToTextApp:
    def __init__(self, root):
        ...
        # GUI Components for Speech Recognition
        ...
        # GUI Components for Translation
        self.translation_lang_var = tk.StringVar()
        self.translation_dropdown = ttk.Combobox(root, textvariable=self.translation_lang_var, width=10)
        self.translation_dropdown['values'] = ['English', 'Russian', 'German']
        self.translation_dropdown.grid(column=2, row=0, padx=5, pady=5)
        self.translation_dropdown.current(0) # Default to English

        self.start_button = tk.Button(root, text="Start", command=self.start_recognition)
        self.start_button.grid(column=3, row=0, padx=5, pady=5)

        self.stop_button = tk.Button(root, text="Stop", command=self.stop_recognition, state='disabled')
        self.stop_button.grid(column=4, row=0, padx=5, pady=5)

        self.text_area = tk.Text(root, height=15, width=80)
        self.text_area.grid(column=0, row=1, columnspan=5, sticky="nsew", padx=5, pady=5)
    ...
    def recognize_speech_from_mic(self, device_index, language):
        ...
        def recognized(args):
            if args.result.text:
                translated_text = self.translate_text(args.result.text)
    ...
    def translate_text(self, text):
        # Mapping GUI language selections to Google Translate language codes
        lang_map = {'English': 'en', 'Russian': 'ru', 'German': 'de'}
        target_lang = lang_map[self.translation_lang_var.get()]
